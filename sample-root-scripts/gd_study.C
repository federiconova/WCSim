...
// 2. export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$WCSIM_BASE_DIR
// 3. g++ -o gd_study.exe gd_study.C `root-config --cflags --glibs`  -L${WCSIM_BASE_DIR} -I${WCSIM_BASE_DIR} -lWCSimRoot
// 4. ./gd_study.exe -v true -f ../wcsim.root
//
...
int main(int argc, char** argv){
  ...
  char* wcsimdirenv;
  wcsimdirenv = getenv("WCSIM_BASE_DIR");
  if(wcsimdirenv !=  NULL){
    gSystem->Load("${WCSIM_BASE_DIR}/libWCSimRoot.so");
