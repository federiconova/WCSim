...
int main(int argc, char** argv){
  ...
  primary_events_tree.Branch("ncapturecount",&ncapturecount,"ncapturecount/I");
  primary_events_tree.Branch("neutroncount",&neutroncount,"neutroncount/I");
  primary_events_tree.Branch("capt_x",capt_x,"capt_x[ncapturecount]/D");
  primary_events_tree.Branch("capt_y",capt_y,"capt_y[ncapturecount]/D");
  primary_events_tree.Branch("capt_z",capt_z,"capt_z[ncapturecount]/D");
  primary_events_tree.Branch("capt_t0",capt_t0,"capt_t0[ncapturecount]/D");
  primary_events_tree.Branch("capt_E",capt_E,"capt_E[ncapturecount]/D");
  primary_events_tree.Branch("capt_num",capt_num,"capt_num[ncapturecount]/I");
  primary_events_tree.Branch("capt_pid",capt_pid,"capt_pid[ncapturecount]/I");
  primary_events_tree.Branch("capt_nucleus",capt_nucleus,"capt_nucleus[ncapturecount]/I");
  primary_events_tree.Branch("capt_nphot",capt_nphot,"capt_nphot[ncapturecount]/I");
  primary_events_tree.Branch("capt_ngamma",capt_ngamma,"capt_ngamma[ncapturecount]/I");
  Int_t nneutrons;
  primary_events_tree.Branch("nneutrons",&nneutrons,"nneutrons/I");

  Int_t count_events = 0;
  Int_t count_numu = 0;
  Int_t count_muons = 0;
  ...
  for(int ievent=0; ievent<n_primary_events; ievent++){ 
	...
	if( wcsimroottrack->GetIpnu() == 13 )
	  count_muons ++;
	else if( wcsimroottrack->GetIpnu() == 14 )
	  count_numu ++;
	...
	v_track_id.push_back(wcsimroottrack->GetId());
      ...
      if(nNeutrons != 0){nEventsWithAtLeastOneNeutron++; eventCaptureMap[ievent] = 0;}
	...
	for(std::map<std::string,int>::iterator iter = nonCaptureNucleus.begin();
      ...
      neutroncount = nNeutrons;
      nneutrons = nNeutrons;
      ncapturecount = nCaptures;
