...
int * corrected_host_times;  // time of a hit
bool create_from_emerald = true;


void create_tree_corrected_times(){
  ...
  std::vector<Float_t> v_digitized_hit_Q, v_digitized_hit_time, v_corrected_hit_time;
  std::vector<std::vector<Float_t> > digitized_hit_Q, digitized_hit_time, corrected_hit_time;
  ...
  primary_events_tree.Branch("corrected_hit_time",&corrected_hit_time);  // The readout digitized time
  ...
  for(int ievent=0; ievent<n_primary_events; ievent++){ 
	...
	v_corrected_hit_time.push_back(corrected_host_times[idigitizedhit]);
	...
	v_digitized_hit_photon_ids.push_back(vv_digitized_hit_photon_ids);
      ...
      corrected_hit_time.push_back(v_corrected_hit_time);
...
bool read_input(){
  ...
  int time, corrected_time;
  double timef, corrected_timef;
  size_t id;
  for( size_t i=0; i<n_hits; i++){
    if( fscanf(f, "%d %lf %lf", &id, &timef, &corrected_timef) != 3 ){
      printf(" problem scanning hit %d from input \n", i);
      fclose(f);
      return false;
    }
    time = int(floor(timef));
    corrected_time = int(floor(corrected_timef));
    host_times[i] = time;
    corrected_host_times[i] = corrected_time;
    host_ids[i] = id;
  }
...
bool read_the_input(){
  ...
  corrected_host_times = (int *)malloc(n_hits*sizeof(int));
