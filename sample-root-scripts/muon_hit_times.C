...
int main(){
  ...
  // create output file
  TFile *of = new TFile("muon_hit_times.root", "RECREATE");
  Float_t time, time0;
  ...
  double time_max = primary_events_tree->GetMaximum("digitized_hit_time");
  ...
  for(int ievent=0; ievent<primary_events_tree->GetEntries(); ievent++){
	...
	time = digitized_hit_time->at(itrigger).at(itime) - time0;
