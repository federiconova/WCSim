...
If used here, uncomment the SetVisAttributes(WClogic) line, and comment out the SetVisAttributes(G4VisAttributes::Invisible) line.*/
  ...
  if( WCPMTName == "x16TriangularTile3inch" ){
	G4double zStep = verticalSpacing/8.;
	G4double yStep = horizontalSpacing/2.;
	G4cout << " qqq WCLength " << WCLength << " WCRadius " << WCRadius << " horizontalSpacing " << horizontalSpacing << " WCPMTperCellHorizontal " << WCPMTperCellHorizontal << " barrelCellWidth " << barrelCellWidth << " verticalSpacing " << verticalSpacing << " WCPMTperCellVertical " << WCPMTperCellVertical << " barrelCellHeight " << barrelCellHeight << " yStep " << yStep << " zStep " << zStep << G4endl;

	for(G4double i = 0; i < WCPMTperCellHorizontal; i++){
	  for(G4double j = 0; j < WCPMTperCellVertical; j++){
		for(G4double k = 0; k < 8; k++){
		  for(G4double l = 0; l < 2; l++){
			G4ThreeVector PMTPosition =  G4ThreeVector(WCIDRadius,
													   -barrelCellWidth/2.+(i+0.5)*horizontalSpacing + (l - 0.5)*yStep,
													   -barrelCellHeight/2.+(j+0.5)*verticalSpacing + (k - 3.5 + l*0.5)*zStep);
			...
			G4VPhysicalVolume* physiWCBarrelPMT =
			  new G4PVPlacement(WCPMTRotation,              // its rotation
							  ...
							  logicWCBarrelCell,         // its mother volume
								...
								(int)(((i*WCPMTperCellVertical+j)*8 + k)*2 + l),
								true);                       
  ...
  if(!(WCBarrelRingNPhi*WCPMTperCellHorizontal == WCBarrelNumPMTHorizontal)){
	...
	if(isODConstructed){
		...
		logicWCODBottomCapTyvek->SetVisAttributes(G4VisAttributes::Invisible);
    ...
    if(WCODPMTShift > barrelODCellWidth/2. - WCPMTODRadius) WCODPMTShift == 0.*cm;
		...
		G4cout << " qqq OD PMTs Barrel Side PMT position (" << PMTPosition.x() << ", " << PMTPosition.y() << ", " << PMTPosition.z() << ") " << G4endl;
		...
		G4cout << " qqq PMTs PMT position (" << PMTPosition.x() << ", " << PMTPosition.y() << ", " << PMTPosition.z() << ") " << G4endl;
...
// used for RayTracer (makes the caps blacksheet yellow)
 ...
 // logicWCPMT->GetDaughter(0),physiCapPMT is the glass face. If you add more 
  ...
  if(isODConstructed){
    ...
    for(G4double i = 0; i < WCPMTODperCellHorizontal; i++){
      for(G4double j = 0; j < WCPMTODperCellVertical; j++){
        G4ThreeVector PMTPosition =  G4ThreeVector(WCODRadius+WCODTyvekSheetThickness,
                                                   -barrelODCellWidth/2.+(i+0.5)*horizontalODSpacing+((G4int)(std::pow(-1,j))*(G4int)(WCODPMTShift)/2),
                                                   -(barrelCellHeight * (barrelODCellWidth/barrelCellWidth))/2.+(j+0.5)*verticalODSpacing);


		G4cout << " qqq OD Barrel PMTs PMT position (" << PMTPosition.x() << ", " << PMTPosition.y() << ", " << PMTPosition.z() << ") " << G4endl;
    ...
    if(!(WCBarrelRingNPhi*WCPMTperCellHorizontal == WCBarrelNumPMTHorizontal)){
      ...
      for(G4double i = 0; i < (WCPMTODperCellHorizontal); i++){
        for(G4double j = 0; j < WCPMTODperCellVertical; j++){
		  ...
		  G4cout << " qqq OD barrel PMT ! PMT position (" << PMTPosition.x() << ", " << PMTPosition.y() << ", " << PMTPosition.z() << ") " << G4endl;
